  ...
  elseif( $action == "new" ):
      if( $_POST ):
        ...
        if( empty($name) || strlen($name) < 5 ){
          $error      = 1;
          $errorText  = "Üye adı en az 5 karakter olmalı";
          $icon     = "error";
          ...
          $error      = 1;
          $errorText  = "Belirttiğiniz telefon numarası kullanılıyor.";
          $icon     = "error";
          ...
          $insert = $conn->prepare("INSERT INTO clients SET name=:name, balance_type=:balance_type, debit_limit=:debit_limit,  username=:username, email=:email, password=:pass, telephone=:phone, register_date=:date, apikey=:key, tel_type=:tel_type, email_type=:email_type, access=:access ");
          $insert = $insert-> execute(array("name"=>$name,"debit_limit"=>$debit_limit,"balance_type"=>$debit,"username"=>$username,"email"=>$email,"pass"=>md5(sha1(md5($pass))),"phone"=>$tel,"date"=>date("Y.m.d H:i:s"),'key'=>$apikey,'tel_type'=>$tel_type,'email_type'=>$email_type,'access'=>json_encode($access) ));
          if( $insert ):
            $conn->commit();
